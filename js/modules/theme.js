window.PhantomTheme = {
  ...
  applyTheme(theme) {
    ...
    themeBtn.innerHTML = isDark ? this.SUN_ICON : this.MOON_ICON;
  },

  initTheme() {
    let savedTheme = 'light';
    try { savedTheme = localStorage.getItem('phantom-theme') || 'light'; } catch (e) { }
    this.applyTheme(savedTheme);

    document.getElementById('themeToggle')?.addEventListener('click', () => {
      const nextTheme = document.documentElement.getAttribute('data-theme') === 'dark' ? 'light' : 'dark';
      this.applyTheme(nextTheme);
      try { localStorage.setItem('phantom-theme', nextTheme); } catch (e) { }
    });
  }
};
